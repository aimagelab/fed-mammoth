...
from global_consts import ADDITIONAL_ARGS
...
def add_args(
    parser: ArgumentParser, model_name: str, network_name: str, dataset_name: str
) -> None:
    ...
    for module in [model, network, dataset]:
        ...
        for arg_name, value in list(signature.parameters.items()):
            if (
                arg_name not in EXCLUDE_FROM_ARGS
                and arg_name != "args"
                and arg_name != "kwds"
            ):
                ...
                if default is Parameter.empty:
                    parser.add_argument(
                        f"--{arg_name}", type=value.annotation, required=True
                    )
                else:
                    parser.add_argument(
                        f"--{arg_name}", type=value.annotation, default=default
                    )
