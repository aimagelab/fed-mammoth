...
class VisionTransformer(nn.Module):
    ...
    def get_scores(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if prompt is not None:
                p_list, score = prompt.get_scores(q, i, x, train=True, task_id=task_id)
...
class PowderPrompt(nn.Module):
    ...
    def forward(self, x_querry, l, x_block, train=False, task_id=None):
        ...
        if l in self.e_layers:
            ...
            if train:
                ...
                #else:
                # in Powder we don't freeze past tasks, we simply use only the avialable prompts for this task.
                K = K[s:f]
                A = A[s:f]
                p = p[s:f]
            # in testing (done by the server) we use all prompts, like in CodaPrompt
            else:
                K = K[0:f]
                A = A[0:f]
                p = p[0:f]
    ...
    def get_scores(self, x_querry, l, x_block, train=False, task_id=None):
        # e prompts
        e_valid = False
        aq_k = None
        if l in self.e_layers:
...
@register_network("vit_prompt_powder")
class ViTZoo(BaseNetwork):
    def __init__(self, model_name:str = "vit_base_patch16_224.augreg_in21k", pretrained:str_to_bool = True, num_tasks: int = 10, num_classes: int = 100, pool_size: int = 100, prompt_length: int = 8):
        ...
        self.prompt = PowderPrompt(768, prompt_param[0], prompt_param[1])
    ...
    def get_scores(self, x, pen=False, train=False, return_alpha=False):
        if self.prompt is not None:
            with torch.no_grad():
                q, _ = self.feat(x)
                q = q[:, 0, :]
                scores = self.feat.get_scores(x, prompt=self.prompt, q=q, train=train, task_id=self.task_id)
                scores = torch.cat([score.unsqueeze(2) for score in scores], dim=2).mean(dim=2)
                return scores #shape (B, num_prompts), ovvero (16, 10) tipicamente
        else:
            raise ValueError("self.prompt is None, therefore prompt scores cannot be computed!")
