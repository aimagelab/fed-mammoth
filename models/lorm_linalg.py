                ...
                if self.cur_train_matrix == "A":
                    ...
                    for key in self.lora_keys:
                        if "weight" in key and self.middle_names.get(key) is not None and not "head" in key:
                            ...
                            clgn = torch.stack([client["grams"][name].to(self.device).to(dtype) for client in client_info])
                            A_s = torch.stack([A_[key].to(self.device).to(dtype) for A_ in cl_A])
                            E = torch.matmul(A_s, clgn).sum(0)
                            # E = torch.stack(
                            #     [
                            ...
                            A = torch.linalg.solve(G, E, left = False)
                    ...
                    for key in self.lora_keys:
                        if "weight" in key and self.middle_names.get(key) is not None and not "head" in key:
                            ...
                            clgn = torch.stack([client["grams"][name].to(self.device).to(dtype) for client in client_info])
                            B_s = torch.stack([B_[key].to(self.device).to(dtype) for B_ in cl_B])
                            E2 = torch.matmul(torch.matmul(B_s, A.unsqueeze(0)), clgn).sum(0)
                            # E2 = torch.stack(
                            #     [
                            ...
                            # )
                            G = A @ clgn.sum(0) @ A.T
                            B = E2 @ torch.linalg.solve(G, A.T, left = False)
                            #B = E2 @ A.T @ G_inv  # B
