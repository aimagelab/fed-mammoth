...
class PiLora(BaseModel):
    ...
    def begin_round_client(self, dataloader: DataLoader, server_info: dict):
        ...
        params = [{"params": list(self.network.model.blocks[0].attn.qkv.parameters()), "lr": self.lr_back}, {"params": list(self.class_protos[self.cur_task]), "lr": self.lr}]
    ...
    def end_round_server(self, client_info: List[dict]):
        with torch.no_grad():
            ...
            setattr(blk, f'linear_a_q_{self.cur_task}.weight', linear_a_q_weights)
            setattr(blk, f'linear_b_q_{self.cur_task}.weight', linear_b_q_weights)
            setattr(blk, f'linear_a_v_{self.cur_task}.weight', linear_a_v_weights)
            setattr(blk, f'linear_b_v_{self.cur_task}.weight', linear_b_v_weights)
